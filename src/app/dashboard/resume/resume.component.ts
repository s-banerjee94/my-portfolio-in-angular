...
export class ResumeComponent implements OnInit {
  ...
  loadResumes(): void {
    this.profileService.getAllResumes().pipe(
      tap((resumes: Resume[]) => {
  ...
  onFileUpload(event: any): void {
    ...
    if (file) {
      this.profileService.uploadFile(file).pipe(
        tap(fileUrl => {
  ...
  saveResume(resume: Resume): void {
    this.profileService.saveResume(resume).pipe(
      tap(() => {
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
  ...
  deleteResume(resume: Resume): void {
    ...
    if (resume.id) {
      this.profileService.deleteResume(resume.id).pipe(
        tap(() => {
          if (resume.fileUrl) {
  ...
  setPrimary(resume: Resume): void {
    if (resume.id) {
      ...
      const updates: Observable<any>[] = this.resumes
        ...
        .map(r => {
          return this.profileService.updateResume(r.id!, { ...r, isPrimary: false });
        });

      updates.push(this.profileService.updateResume(resume.id, { ...resume, isPrimary: true }));

      if (updates.length > 0) {
        this.profileService.updateResume(resume.id, { ...resume, isPrimary: true }).pipe(
          tap(() => {
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
